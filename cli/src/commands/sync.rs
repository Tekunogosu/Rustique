...
use core::api::api_structs::{Mod, ModsSearchFile};
use core::api::client::{ApiClient};
use core::rustique_errors::RustiqueError;
use core::utils::{prettify_json,extract_all_mods_metadata, find_mod_id, get_current_time, split_modid_version, parse_json_file, timestamp_older_than, write_json_file};
use core::version_management::{parse_latest_version, parse_pinned_version, parse_version};
...
use core::config::config_manager::{get_config, Config, Package};
use core::consts::{FILE_GAME_VERSION_SYNC, FILE_MOD_SEARCH_SYNC, FILE_RUSTIQUE_SYNC};
use core::information_utils::{display_table, elapsed_footer, notice, CellData};
use core::symlink_manager::SymlinkManager;
use core::traits::ref_ext::PathRef;
use core::sync_structs::{RustiqueSyncJson, ModSyncInfo, GameVersionSync};
...
pub async fn daily_file_syncs(force: bool) -> Result<ModsSearchFile, RustiqueError> {
    ...
    if file_data.mods.is_empty() || force || timestamp_older_than(sync_time, &file_data.last_sync){
        ...
        let json = prettify_json(&file_data, "Mods Search DB")?;
...
pub async fn game_version_sync(force: bool) -> Result<GameVersionSync, RustiqueError> {
    ...
    if file_data.game_versions.is_empty() || force || timestamp_older_than(sync_time, &file_data.last_sync){
        ...
        let json = prettify_json(&file_data, "Game Version Sync")?;
